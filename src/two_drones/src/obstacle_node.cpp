...
drone3.fix = n.subscribe("drone3/fix", 10, &drone3_gps_callback);
...
drone3.magnetic = n.subscribe("drone3/magnetic", 10, &Drone_Mission::magnetic_callback, &drone3);
...
drone3.imu = n.subscribe("drone3/raw_imu", 10, &Drone_Mission::imuCallback, &drone3);
...
drone3.odom = n.subscribe("drone3/odom", 10, &Drone_Mission::odomCallback, &drone3);
...
drone3.motor_on = n.serviceClient<hector_uav_msgs::EnableMotors>("drone3/enable_motors");
...
drone3.move_drone = n.advertise<geometry_msgs::Twist>("drone3/cmd_vel", 1);
...
drone3.tester = n.advertise<geometry_msgs::Vector3>("drone3/yaw_vector", 1);
...
void step(Drone_Mission &drone) {
  ...
  if(drone.state == 1 && drone.finished != true) {
    ...
    } else if(drone.name == "drone3") {
      Pose pose_("drone3/action/pose", true);
      drone3_maintain_altitude(4, pose_);
    } else {  
...
void drone3_maintain_altitude(float height3, Pose &pose3_) {
  ROS_INFO("Waiting for action server to start.");
  // wait for the action server to start
  pose3_.waitForServer();
  ROS_INFO("Action server started, sending goal.");
  drone3.pose_goal.target_pose = drone3.pose_feedback.feedback.current_pose;
  drone3.pose_goal.target_pose.pose.position.x = 0;
  drone3.pose_goal.target_pose.pose.position.y = 0;
  drone3.pose_goal.target_pose.pose.position.z = height3;
  ROS_INFO("Goal prepared");
  drone3.pose_goal.target_pose.header.frame_id = "drone3/world";
  pose3_.sendGoal(drone3.pose_goal, &doneCb, &activeCb, &feedbackCb);
  ROS_INFO("Goal Sent");
  // pose3_.waitForResult(ros::Duration(30.0));

  if (pose3_.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
  {
      actionlib::SimpleClientGoalState state = pose3_.getState();
      ROS_INFO("Action finished: %s",state.toString().c_str());
  }
  else
      ROS_INFO("Action did not finish before the time out."); 
...
void drone3_gps_callback(const sensor_msgs::NavSatFix::ConstPtr& msg) {
  ...
  geodesy::convert(drone3.current_gps, drone3.gp);
  geodesy::fromMsg(drone3.gp, drone3.current_utm);
  switch(drone3.state) 
  {
    case 0: 
      // ROS_INFO("Case 0");
    ...
    case 1:
      if(drone3.finished == false) {
        if(drone1.rotateToHeading && drone2.rotateToHeading)
          step(drone3);
      } else {
        drone3.state = 2;
